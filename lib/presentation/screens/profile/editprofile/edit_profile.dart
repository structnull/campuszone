...
class _EditProfilePageState extends State<EditProfilePage> {
  ...
  Future<void> _updateUserData() async {
    ...
    // Validate inputs if needed (AppTextField handles some, but form wrapper is good)

    setState(() => _isLoading = true);

    try {
      // We need existing user to copyWith, or just create minimal update model
      // Since repo updateUser takes UserModel, we ideally should get current, update fields, save.
      // But for update simple fields, we can probably construct "partial" user if datasource supports it,
      // or we must refetch.
      // Optimized: user entered data assumes full overwrite of these fields.
      // Ideally we fetch current again or store it in state.
      // Let's assume we re-fetching or we stored 'currentUserModel' in state.
      // For simplicity here, I'll fetch current first to get createdAt/etc which are required by UserModel constructor.
